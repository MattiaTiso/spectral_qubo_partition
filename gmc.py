...
def _update_similarity(E_input, U, view_weight, k_nn):
    ...
    numerator = e_next[:, None] - e_near + 2.0 * view_weight * (u_near - u_next[:, None])
    denominator = (
...
class GMC:
    ...
    def __init__(self, k, k_nn=5, max_iter=50, lam_init=1.0,
                 lam_factor=2.0, tol=1e-8, verbose=False):
        self.k = int(k)
        self.k_nn = int(k_nn)
    ...
    def fit_distances(self, distance_views):
        ...
        for iteration in range(self.max_iter):
            S_list = [_update_similarity(E_list[v], U, float(weights[v]), k_nn)
            ...
            if components < self.k:
                ...
                regularization /= self.lam_factor
            self.history_.append({"iter": iteration + 1,
                                  "n_components": components,
                                  "lambda": regularization,
                                  "weights": weights.copy(),
                                  "eigenvalues": eigenvalues.copy()})
            if self.verbose:
                print(f"[GMC-CPU] iter={iteration+1} components={components} "
                      f"lambda={regularization:.6g}")
            if components == self.k:
                break
        labels, found = connected_components_labels(U, self.tol)
        if found < self.k:
            warnings.warn(f"GMC: {found} componenti invece di {self.k}; fallback k-means.",
                          stacklevel=2)
...
class BinaryHierarchicalGMC:
    """Binary GMC preserving global row features via precomputed distances.

    ``E_global[idx, idx]`` is exactly the distance matrix obtained from
    ``X_global[idx, :]``. It is not equivalent to computing distances from
    ``X_global[np.ix_(idx, idx)]``.
    """
    def __init__(self, k, gmc_k_nn=5, gmc_max_iter=50, gmc_tol=1e-8,
                 verbose=False):
        if k < 1 or k & (k - 1):
    ...
    def fit(self, global_feature_views):
        ...
        # Pay O(n^2 p_v) once per view; child nodes reuse exact global distances.
        ...
        for level in range(self.depth_):
            if self.verbose:
                print(f"[Binary-CPU] level={level} nodes={len(current_ids)}")
            child_ids = []
            for node_id in current_ids:
                node = self.nodes_[node_id]
                idx = node.global_idx
                # Exact equivalent of selecting X[idx, :] and recomputing distances.
                node_distances = [E[np.ix_(idx, idx)] for E in self.distance_views_]
                model = GMC(k=2, k_nn=self.gmc_k_nn,
                            max_iter=self.gmc_max_iter,
                            tol=self.gmc_tol, verbose=False).fit_distances(node_distances)
                left_global = idx[np.flatnonzero(model.labels_ == 0)]
                right_global = idx[np.flatnonzero(model.labels_ == 1)]
                left = BinaryTreeNode(next_id, level + 1, left_global, node_id)
                node.left_child_id = next_id
                self.nodes_[next_id] = left
                child_ids.append(next_id)
                next_id += 1
                right = BinaryTreeNode(next_id, level + 1, right_global, node_id)
                node.right_child_id = next_id
                self.nodes_[next_id] = right
                child_ids.append(next_id)
                next_id += 1
            current_ids = child_ids
        leaves = sorted((node for node in self.nodes_.values()
                         if node.left_child_id is None), key=lambda node: node.node_id)
