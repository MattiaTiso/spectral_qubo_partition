...
def project_simplex_rows_gpu(V):
    ...
    cumulative_sum = (
        cp.cumsum(sorted_values, axis=1) - 1.0
    )
...
def _initialize_similarity_gpu(E_input, k_nn):
    ...
    raw[safe] = (
        next_distance[safe, None]
        - near[safe]
    ) / denominator[safe, None]

    weights = project_simplex_rows_gpu(
        cp.maximum(raw, 0.0)
    )
...
def _update_similarity_gpu(
    ...
    denominator = (
        k_nn * e_next- e_near.sum(axis=1)- 2.0* k_nn* view_weight* u_next+ 2.0* view_weight* u_near.sum(axis=1)
    )
...
def _update_u_gpu(
    ...
    for view_id, similarity in enumerate(similarities):
        ...
        candidate += (
            similarity
            - coefficient * spectral_distances
        )
...
class GMCGPU:
    ...
    def gpu_dtype(self):
        ...
        raise ValueError(
            "dtype deve essere 'float32' oppure 'float64'."
        )
    ...
    def fit_distances(self, distance_views):
        ...
        for view_id, distance_view in enumerate(
            distance_views
        ):
        ...
        for view_id, similarity in enumerate(
            similarities
        ):
            U += (
                weights[view_id]
...
class BinaryHierarchicalGMCGPU:
    ...
    def _resolved_mode(self):
        """
        Determina la modalità effettiva di esecuzione.
        """
    ...
    def _fit_node_gpu(
        ...
        -------
        tuple
            node_id, left_indices, right_indices
        """
        ...
        # Imposta esplicitamente il device anche nel worker.
        with cp.cuda.Device(device_id):
            # Tutte le operazioni CuPy eseguite nel blocco
            # vengono accodate nello stream del worker.
            with stream:
                ...
                # Garantisce che tutte le operazioni
                # dello stream siano terminate prima
                # di lasciare il worker.
                stream.synchronize()

        left_indices = global_indices[
            model.labels_ == 0
        ]

        right_indices = global_indices[
            model.labels_ == 1
        ]
    ...
    def fit(self, global_feature_views):
        ...
        views, n = validate_feature_views(
        ...
        if mode == "serial":
            ...
            self.distance_views_ = (
                reference.distance_views_
            )
        ...
        # GPU visibile nel processo corrente.
        # Con SLURM e CUDA_VISIBLE_DEVICES, normalmente
        # questo identificatore è 0.
        device_id = int(
        ...
        if self.verbose:
            ...
            print(
                f"[Binary-GPU] device={device_id} "
                f"name={device_name} "
                f"max_parallel_nodes="
                f"{self.max_parallel_nodes}"
            )
        ...
        for level in range(self.depth_):
            ...
            number_of_nodes = len(current_ids)
